...
import { publishCVToGitHub } from "./githubPublisher";
...
${JSON.stringify({
  ...
  // Dynamic Route: Server-side rendering of the beautiful Hosted CV pagecd web1
  ...
  // API: Publish a CV to GitHub Pages via the CV_file repository
  app.post("/api/publish-cv/:username", async (req, res) => {
    const username = req.params.username.toLowerCase();
    try {
      // Reuse the already-working /cv/:username renderer internally,
      // instead of duplicating the HTML generation logic.
      const selfUrl = `http://localhost:${PORT}/cv/${username}`;
      const htmlResponse = await fetch(selfUrl);

      if (!htmlResponse.ok) {
        return res.status(404).json({ error: "CV not found. Please save your CV first before publishing." });
      }

      const html = await htmlResponse.text();
      const result = await publishCVToGitHub(username, html);

      res.json({ success: true, url: result.url });
    } catch (err: any) {
      console.error("Failed to publish CV to GitHub:", err);
      res.status(500).json({ error: "Failed to publish CV: " + (err.message || String(err)) });
    }
  });
   app.get("*", (req, res) => {
