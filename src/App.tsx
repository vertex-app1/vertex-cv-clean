...
const [isPublishingGithub, setIsPublishingGithub] = useState(false);
  const [githubPublishUrl, setGithubPublishUrl] = useState("");
  const [githubPublishError, setGithubPublishError] = useState("");
...
if (data.url) {
  ...
  const handlePublishToGithub = async () => {
    if (!username.trim()) {
      setGithubPublishError("Please save your CV first before publishing to GitHub.");
      return;
    }
    ...
    try {
      const res = await fetch(`/api/publish-cv/${username.trim().toLowerCase()}`, {
        method: "POST"
      });
      const data = await res.json();
      if (data.success && data.url) {
        setGithubPublishUrl(data.url);
      } else if (data.error) {
        setGithubPublishError(data.error);
      }
    } catch (e) {
      console.error(e);
      setGithubPublishError("Failed to publish to GitHub. Could not communicate with server.");
    } finally {
              ...
              {generatedUrl && (
                <div className="space-y-3 pt-2">
                  <button
                    ...
                    className="w-full py-2.5 rounded-lg bg-gradient-to-r from-gray-700 to-gray-900 text-white font-bold text-xs flex items-center justify-center gap-2 hover:opacity-95 cursor-pointer active:scale-[0.98] transition-all disabled:opacity-50 shadow-lg"
                  >
                    {isPublishingGithub ? (
                      <>
                        <RefreshCw className="w-3.5 h-3.5 animate-spin" />
                        {dashboardLang === "ar" ? "جاري النشر على GitHub..." : "Publishing to GitHub..."}
                      </>
                    ) : (
                      <>
                        <Save className="w-3.5 h-3.5" />
                  ...
                  {githubPublishUrl && (
                    ...
                    >
                      <p className="text-xs font-bold text-emerald-400 flex items-center justify-center gap-1">
                        ...
                        {dashboardLang === "ar" ? "تم النشر على GitHub Pages بنجاح!" : "Published to GitHub Pages!"}
                      </p>
                      <div className="p-2.5 bg-black/40 border border-emerald-500/10 rounded-lg font-mono text-[10px] break-all select-all text-emerald-300">
                        {githubPublishUrl}
                      </div>
                      <a 
                        href={githubPublishUrl} 
                        target="_blank" 
                        rel="noreferrer"
                        className={`inline-flex items-center gap-1.5 px-4 py-2 text-xs font-bold text-white bg-gradient-to-r ${themeColors.buttonBg} rounded-lg hover:opacity-90 cursor-pointer active:scale-[0.98] transition-all shadow-md ${themeColors.glowShadow}`}
                      >
                        {dashboardLang === "ar" ? "فتح الصفحة على GitHub Pages" : "Open GitHub Pages Site"}
                      </a>
                    </motion.div>
                  )}
                </div>
